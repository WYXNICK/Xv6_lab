...
pagetable_t kvm_init_once()
{
    ...
    // 映射 UART 寄存器
    kvmmap_one(newpg, UART0, UART0, PGSIZE, PTE_R | PTE_W);
    // 映射 virtio mmio 磁盘接口
    kvmmap_one(newpg, VIRTIO0, VIRTIO0, PGSIZE, PTE_R | PTE_W);
    // 映射 CLINT
    kvmmap_one(newpg, CLINT, CLINT, 0x10000, PTE_R | PTE_W);
    // 映射 PLIC
    kvmmap_one(newpg, PLIC, PLIC, 0x400000, PTE_R | PTE_W);
    // 映射内核文本，设置为可执行和只读
    kvmmap_one(newpg, KERNBASE, KERNBASE, (uint64)etext - KERNBASE, PTE_R | PTE_X);
    // 映射内核数据和物理 RAM
    kvmmap_one(newpg, (uint64)etext, (uint64)etext, PHYSTOP - (uint64)etext, PTE_R | PTE_W);
    // 映射陷阱入口/出口的跳板到内核的最高虚拟地址
    kvmmap_one(newpg, TRAMPOLINE, (uint64)trampoline, PGSIZE, PTE_R | PTE_X);
    return newpg;
}
